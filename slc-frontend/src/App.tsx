                ...
                <main style={main}>
                    ...
                    <section style={panel}>
                        ...
                        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                            <input placeholder="Enter properties separated by spaces or commas" value={input} onChange={(e) => setInput(e.target.value)} style={{ ...inputStyle, flex: 1 }}/>
                            <button onClick={onClassify} disabled={!inputList.length || busy} style={btnPrimary}>Classify</button>
                        </div>
                        ...
                        {prediction && (
                            <div style={card}>
                                ...
                                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 8 }}>
                                    <button onClick={() => sendFeedback("class1")} disabled={busy} style={btnPrimary}>It is {snap?.class1?.name || c1 || "Class 1"}</button>
                                    <button onClick={() => sendFeedback("class2")} disabled={busy} style={btnPrimary}>It is {snap?.class2?.name || c2 || "Class 2"}</button>
                                    <button onClick={() => sendFeedback("none")} disabled={busy} style={btnOutline}>None class</button>
                                </div>
                            </div>
                        )}
                    </section>
